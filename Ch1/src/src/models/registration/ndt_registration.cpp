...
bool NDTRegistration::SetRegistrationParam(float res, float step_size, float trans_eps, int max_iter) {
    ...
    ndt_ptr_->setTransformationEpsilon(trans_eps);      // setTransformationEpsilon 设置两次变换矩阵之间的差值（一般设置为1e-10）
...
bool NDTRegistration::ScanMatch(const CloudData::CLOUD_PTR& input_source, 
    ...
    ndt_ptr_->align(*result_cloud_ptr, predict_pose);       // param1：配准后的source点云，param2：预测的pose
    result_pose = ndt_ptr_->getFinalTransformation();       // 获取最终的pose
