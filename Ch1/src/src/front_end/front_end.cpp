...
bool FrontEnd::InitWithConfig() {
    ...
    InitRegistration(registration_ptr_, config_node);       // 初始化配准方式，基类指针指向派生类对象
...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    {
        registration_ptr = std::make_shared<NDTRegistration>(config_node[registration_method]);     // 多态
    } 
    ...
    {
        registration_ptr = std::make_shared<MY_ICPRegistration>(config_node[registration_method]);
    }
...
// 把初始化的GNSS+IMU pose赋值给第一帧的pose，并将其赋值给第二帧的predict_pose
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    global_map_frames_.push_back(key_frame);        // 仅仅是为了计数，不占内存
...
bool FrontEnd::GetNewGlobalMap(CloudData::CLOUD_PTR& global_map_ptr) {
    if (has_new_global_map_) {
        ...
        global_map_ptr_.reset(new CloudData::CLOUD());      // 释放内存
        return true;
    }
    return false;
}
